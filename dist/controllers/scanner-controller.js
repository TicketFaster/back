...
class ScannerController {
    ...
    async markBilletAsUsed(req, res) {
        try {
            ...
            logger_utility_1.logger.error(`Erreur lors du marquage du billet ${req.params.id} comme utilisé`, error);
    ...
    async getUsageStatistics(req, res) {
        try {
            const seanceId = req.query.seanceId ? parseInt(req.query.seanceId) : undefined;
            const evenementId = req.query.evenementId ? parseInt(req.query.evenementId) : undefined;
            // Créer un filtre pour les billets
            const filtres = {};
            if (seanceId)
                filtres.seanceId = seanceId;
            ...
            logger_utility_1.logger.error('Erreur lors de la récupération des statistiques d\'utilisation', error);
    ...
    async scanAndValidate(req, res) {
        try {
            ...
            logger_utility_1.logger.error('Erreur lors du scan et de la validation du billet', error);
