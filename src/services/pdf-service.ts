...
export class PDFService {
   ...
   */
  async generateBilletPDF(billet: Billet): Promise<string> {
    try {
      const pdfPath = path.join(this.tempDir, `billet_${billet.id}_${uuidv4()}.pdf`);
      ...
      if (billet.code_barre) {
        const qrCodeDataUrl = await QRCode.toDataURL(billet.code_barre);
      ...
      logger.error(`Erreur lors de la génération du PDF pour le billet ${billet.id}`, error);
   ...
   * @returns Le chemin du fichier PDF généré
   */
  async generateMultipleBilletsPDF(billets: Billet[]): Promise<string> {
    try {
      ...
      const pdfPath = path.join(this.tempDir, `reservation_${reservationId}_${uuidv4()}.pdf`);
      const doc = new PDFDocument({ size: 'A4', margin: 50 });
      ...
      logger.error('Erreur lors de la génération du PDF pour les billets', error);
   ...
   * @returns Le chemin du fichier PDF généré
   */
  async generateRapportVentesPDF(rapportData: any[]): Promise<string> {
    try {
      const pdfPath = path.join(this.tempDir, `rapport_ventes_${uuidv4()}.pdf`);
      const doc = new PDFDocument({ size: 'A4', margin: 50 });
      ...
      logger.error('Erreur lors de la génération du PDF du rapport de ventes', error);
