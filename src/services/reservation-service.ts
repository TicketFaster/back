...
interface ModificationReservation {
  statut_paiement?: string;
  billets?: {
    ajouter?: BilletData[];
    supprimer?: number[];
    modifier?: { id: number; statut: string }[];
  };
}
...
export class ReservationService {
  ...
  async getAllReservations(filtres?: { 
    ...
    try {
      ...
      if (filtres) {
        if (filtres.clientId) {
          query = query.andWhere('reservation.id_client = :clientId', 
            { clientId: filtres.clientId });
        }
        
        if (filtres.statut) {
          query = query.andWhere('reservation.statut_paiement = :statut', 
            { statut: filtres.statut });
        }
        
        if (filtres.dateMin) {
          query = query.andWhere('reservation.date_reservation >= :dateMin', 
            { dateMin: filtres.dateMin });
        }
        
        if (filtres.dateMax) {
          query = query.andWhere('reservation.date_reservation <= :dateMax', 
            { dateMax: filtres.dateMax });
        }
      }
  ...
  async getReservationById(id: number) {
    try {
      ...
      logger.error(`Erreur lors de la récupération de la réservation ${id}: ${error}`);
  ...
  async createReservation(data: ReservationData) {
    ...
    try {
      ...
      for (const billetData of data.billets) {
        ...
        if (!billetData.prix_final) {
          ...
          const prixFinal = await AppDataSource.query(
            ...
            [prixBase, billetData.type_tarif]
          );
      ...
      for (const billetData of data.billets) {
        ...
        if (!prixFinal) {
          const prixBase = seance.evenement.prix_standard;
          const resultatPrix = await AppDataSource.query(
            'SELECT fn_CalculPrixReduit($1, $2) as prix', 
            [prixBase, billetData.type_tarif]
          );
      ...
      const updatedReservation = await queryRunner.manager.findOne(Reservation, {
   ...
   */
  async updateReservation(id: number, data: ModificationReservation) {
    // Utiliser une transaction pour garantir l'intégrité des données
    const queryRunner = AppDataSource.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    ...
    try {
      ...
      // Annulation de la transaction en cas d'erreur
      await queryRunner.rollbackTransaction();
      logger.error(`Erreur lors de la mise à jour de la réservation ${id}: ${error}`);
      throw error;
    } finally {
      // Libération du queryRunner
      await queryRunner.release();
  ...
  async cancelReservation(id: number) {
    ...
    try {
      ...
      // Annulation de la transaction en cas d'erreur
      await queryRunner.rollbackTransaction();
      logger.error(`Erreur lors de l'annulation de la réservation ${id}: ${error}`);
      throw error;
    } finally {
      // Libération du queryRunner
      await queryRunner.release();
  ...
  async deleteReservation(id: number) {
    ...
    try {
      ...
      // Annulation de la transaction en cas d'erreur
      await queryRunner.rollbackTransaction();
      logger.error(`Erreur lors de la suppression de la réservation ${id}: ${error}`);
      throw error;
    } finally {
      // Libération du queryRunner
      await queryRunner.release();
